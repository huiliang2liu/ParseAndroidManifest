...
public class ParseXml {
	...
	private int stringChunkOffset = 8;
	private int resourceChunkOffset;
	private int nextChunkOffset;
	public ArrayList<String> stringContentList;
	public StringBuilder xmlSb = new StringBuilder();
	public HashMap<String, String> uriPrefixMap = new HashMap<String, String>();
	public HashMap<String, String> prefixUriMap = new HashMap<String, String>();
	...
	public ParseXml(byte[] byteSrc) {
		// TODO Auto-generated constructor stub
		parseXmlHeader(byteSrc);
		parseStringChunk(byteSrc);
		parseResourceChunk(byteSrc);
	 ...
	 * @param byteSrc
	 */
	private void parseXmlHeader(byte[] byteSrc) {
	 ...
	 * @param byteSrc
	 */
	private void parseStringChunk(byte[] byteSrc) {
		// String Chunk的标示
		byte[] chunkTagByte = Utils.copyByte(byteSrc, stringChunkOffset, 4);
		// System.out.println("string chunktag:"
		// + Utils.bytesToHexString(chunkTagByte));
		// String Size
		...
		// String Content
		byte[] chunkStringContentByte = Utils.copyByte(byteSrc,
				stringContentStart, chunkSize);

		/**
		 * 在解析字符串的时候有个问题，就是编码：UTF-8和UTF-16,如果是UTF-8的话是以00结尾的，如果是UTF-16的话以00
		 * 00结尾的
		 */
		...
		byte[] firstStringSizeByte = Utils.copyByte(chunkStringContentByte, 0,
				2);
		// 一个字符对应两个字节
		int firstStringSize = Utils.byte2Short(firstStringSizeByte) * 2;
		// System.out.println("size:" + firstStringSize);
		byte[] firstStringContentByte = Utils.copyByte(chunkStringContentByte,
				2, firstStringSize + 2);
		...
		// + Utils.filterStringNull(firstStringContent));

		// 将字符串都放到ArrayList中
		int endStringIndex = 2 + firstStringSize + 2;
		while (stringContentList.size() < chunkStringCount) {
			// 一个字符对应两个字节，所以要乘以2
			int stringSize = Utils.byte2Short(Utils.copyByte(
					chunkStringContentByte, endStringIndex, 2)) * 2;
			String str = new String(Utils.copyByte(chunkStringContentByte,
		 ...
		 */
		/*
		 * int stringStart = 0; int index = 0; while(index < chunkStringCount){
		 * byte[] stringSizeByte = Utils.copyByte(chunkStringContentByte,
		 * stringStart, 2); int stringSize = (stringSizeByte[1] & 0x7F);
		 * System.out
		 * .println("string size:"+Utils.bytesToHexString(Utils.int2Byte
		 * (stringSize))); if(stringSize != 0){ //这里注意是UTF-8编码的 String val = "";
		 * try{ val = new String(Utils.copyByte(chunkStringContentByte,
		 * stringStart+2, stringSize), "utf-8"); }catch(Exception e){
		 * System.out.println("string encode error:"+e.toString()); }
		 * stringContentList.add(val); }else{ stringContentList.add(""); }
		 * stringStart += (stringSize+3); index++; }
		 * 
		 * for(String str : stringContentList){ System.out.println("str:"+str);
		 * }
		 */
	 ...
	 * @param byteSrc
	 */
	private void parseStartNamespaceChunk(byte[] byteSrc) {
		// 获取ChunkTag
	 ...
	 * @param byteSrc
	 */
	private void parseStartTagChunk(byte[] byteSrc) {
		// 解析ChunkTag
		byte[] chunkTagByte = Utils.copyByte(byteSrc, 0, 4);
		// System.out.println(Utils.bytesToHexString(chunkTagByte));
		...
		// + stringContentList.get(prefixIndex));
		// } else {
		...
		// 解析属性
		// 这里需要注意的是每个属性单元都是由五个元素组成，每个元素占用四个字节：namespaceuri, name, valuestring,
		// type, data
		// 在获取到type值的时候需要右移24位
		ArrayList<AttributeData> attrList = new ArrayList<AttributeData>(
				attrCount);
		for (int i = 0; i < attrCount; i++) {
			Integer[] values = new Integer[5];
			AttributeData attrData = new AttributeData(this);
			for (int j = 0; j < 5; j++) {
				int value = Utils.byte2int(Utils.copyByte(byteSrc, 36 + i * 20
						+ j * 4, 4));
				switch (j) {
				case 0:
		...
		// + stringContentList.get(attrList.get(i).nameSpaceUri));
		// } else {
		// System.out.println("nameSpaceUri == null");
		// }
		// if (attrList.get(i).name != -1) {
		// System.out.println("name:"
		// + stringContentList.get(attrList.get(i).name));
		// } else {
		// System.out.println("name == null");
		// }
		// if (attrList.get(i).valueString != -1) {
		// System.out.println("valueString:"
		// + stringContentList.get(attrList.get(i).valueString));
		// } else {
		// System.out.println("valueString == null");
		// }
		// System.out.println("type:"
		// + AttributeType.getAttrType(attrList.get(i).type));
		// System.out.println("data:"
		// + AttributeType.getAttributeData(attrList.get(i), this));
		// }
	 ...
	 * @param byteSrc
	 */
	private void parseEndTagChunk(byte[] byteSrc) {
		byte[] chunkTagByte = Utils.copyByte(byteSrc, 0, 4);
		// System.out.println(Utils.bytesToHexString(chunkTagByte));
		byte[] chunkSizeByte = Utils.copyByte(byteSrc, 4, 4);
		int chunkSize = Utils.byte2int(chunkSizeByte);
	 ...
	 * @param byteSrc
	 */
	private void parseTextChunk(byte[] byteSrc) {
		byte[] chunkTagByte = Utils.copyByte(byteSrc, 0, 4);
		// System.out.println(Utils.bytesToHexString(chunkTagByte));
		byte[] chunkSizeByte = Utils.copyByte(byteSrc, 4, 4);
		int chunkSize = Utils.byte2int(chunkSizeByte);
	 ...
	 * 把构造好的xml写入文件中
	 */
	public void writeFormatXmlToFile() {
		FileWriter fw = null;
		try {
			fw = new FileWriter("xmltest/AndroidManifest_format.xml");
			fw.write(xmlSb.toString());
		} catch (Exception e) {
			// System.out.println("write format xml file error:" +
			// e.toString());
		} finally {
			try {
				fw.close();
	 ...
	 * @return
	 */
	private String createStartTagXml(String tagName,
			List<AttributeData> attrList) {
		if (attrList == null || attrList.size() <= 0)
			return "";
		if ("manifest".equals(tagName)) {
			for (int i = 0; i < attrList.size(); i++) {
				AttributeData data = attrList.get(i);
				if ("versionCode".equals(data.getName()))
					versionCode = data.getData();
				else if ("versionName".equals(data.getName()))
					versionName = data.getData();
				else if ("package".equals(data.getName()))
					packageName = data.getData();
			}
		} else if ("uses-permission".equals(tagName)) {
			if (permissions == null)
				permissions = new ArrayList<String>();
			permissions.add(attrList.get(0).getData());
			...
			meta_data.put(attrList.get(0).getData(),
					attrList.size() > 1 ? attrList.get(1).getData() : "");
		} else if ("service".equals(tagName)) {
			if (service == null)
				service = new ArrayList<String>();
			service.add(attrList.get(0).getData());
		} else if ("receiver".equals(tagName)) {
			if (receiver == null)
				receiver = new ArrayList<String>();
			receiver.add(attrList.get(0).getData());
		...
		if (attrList.size() == 0) {
			...
			for (int i = 0; i < attrList.size(); i++) {
				...
				tagSb.append("    ");
				tagSb.append(prefixName + (prefixName.length() > 0 ? ":" : "")
						+ attr.getName() + "=");
				tagSb.append("\"" + AttributeType.getAttributeData(attr, this)
						+ "\"");
	 ...
	 * @return
	 */
	private boolean isEnd(int totalLen) {
		return nextChunkOffset >= totalLen;
	}

	public String getStringContent(int index) {
		if (stringContentList.size() > index)
